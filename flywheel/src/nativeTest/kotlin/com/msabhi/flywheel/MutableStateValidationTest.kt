...
class MutableStateValidationTest {
    ...
    @Test
    fun mutableStateShouldFail() = runBlocking {
        ...
        val stateReserve = getStateReserve(scope = scope, initialState = initialState, reduce = reduce)
        stateReserve.dispatch(UpdateDataAction(Pair("1", "two")))
        delay(50) // allow background thread to process the action and trigger the exception
